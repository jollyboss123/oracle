...
public class OpenTelemetryConfig {
    ...
    SdkLoggerProvider otelSdkLoggerProvider(ObjectProvider<LogRecordProcessor> logRecordProcessors) {
        Resource springResource = Resource.create(Attributes.of(ResourceAttributes.SERVICE_NAME, applicatioName));
        SdkLoggerProviderBuilder builder = SdkLoggerProvider.builder()
                .setResource(Resource.getDefault().merge(springResource));
        logRecordProcessors.orderedStream()
                .forEach(builder::addLogRecordProcessor);
